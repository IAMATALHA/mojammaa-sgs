/**
 * Backfill des agrégats classStats/{academicYear}__{classe}__{semestre} à partir des notes
...
async function main() {
  ...
  // Grouper par (année scolaire, classe, semestre)
  const groups = new Map()
  snap.forEach((d) => {
    const x = d.data()
    if (!x.academicYear || !x.classe || !x.semestre) return
    const key = `${x.academicYear}|${x.classe}|${x.semestre}`
    if (!groups.has(key)) groups.set(key, [])
    groups.get(key).push(x)
  })

  for (const [key, docs] of groups) {
    const [academicYear, classe, semestre] = key.split('|')
    const stats = computeClassStats(docs)
    const id = statsDocId(academicYear, classe, semestre)
    ...
    if (COMMIT) {
      await db.collection('classStats').doc(id).set({ academicYear, classe, semestre, ...stats, updatedAt: new Date() })
      console.log('  ✓ écrit')
    }
  }
